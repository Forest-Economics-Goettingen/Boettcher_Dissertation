...
stk %>% mutate(Stamm_Krone = ifelse(Stamm_Krone == "S", 0, 1)) %>% 
  ...
  geom_jitter(alpha = .15, width = .16, height = .01) +
...
hist(resid(logit.sk2, type = "response")) # Nicht besser

logit.sk <- datenFuellen(dat = logit.sk, modell = logit.sk1, logit = TRUE)
...
ges <- ges %>% 
  mutate(Sortiment = ifelse(Sortiment == 0, "IS", "SH")) %>% 
