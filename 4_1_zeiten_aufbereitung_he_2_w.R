...
#### Daten einlesen ####
sqlStatement1 <- paste0("SELECT * FROM ",flaeche,"_stk_w")
sqlStatement2 <- paste0("SELECT * FROM ",flaeche,"_baum_w")
...
# Test Kai #
test1$Sorten <- test1 %>% select(Sorten.x : Sorten.y.y) %>% apply(1, function(x) {(paste(x[!is.na(x)], collapse = "-"))})
...
# plot normal Zeit_N # 
baum %>% 
  ggplot(aes(x=Volumen_stk, y=Sortimente+Umgreifen, color = as.factor(Sorten)))+
  geom_point(alpha = 0.6)+
  geom_smooth(se = FALSE,size = 2, method = "glm", formula = y~x)+
...
baum %>% 
  ggplot(aes(x=BHD_cm, y=Faellung, color = Verlauf_Faellung))+
  geom_point(alpha = 0.6)+
