...
hist(resid(gam.1, type = "response")) # besser
hist(resid(step.lm.1))

# Ausfüllen der Zeit_N Werte, hier nicht notwendig, alle Zeitwerte vorhanden ##
...
test2 <- tibble(Baum_Nr = stk$Baum_Nr[stk$Sortiment %in% "SH"] %>% unique(), Sorten = "SH-IS")
baum <- left_join(baum, test2, by = "Baum_Nr")
baum$Sorten[is.na(baum$Sorten) & !is.na(baum$Sortimente)] <- "IS" 

baum <- baum %>% 
  mutate(Anzahl_Sorten = ifelse(Sorten == "SH-IS", 2, 1))
...
## Greifen  ##
baum %>% filter(Verlauf_Greifen == "N") %>% 
  ggplot(aes(x=Volumen_stk, y=Greifen))+
  geom_point(alpha = 0.6)+
  geom_smooth(se = T,size = 2, method = "glm", formula = y~x)+
  #labs(x="G", y="Processing time [sec./piece]", title = "Tree processing - linear")+
  theme_gray(base_size = 15)+
  theme(legend.position = "none")
...
lm.b2 <- glm(Greifen ~ Volumen_stk, data = baum[baum$Verlauf_Greifen == "N",])
summary(lm.b2)
...
# Offenbar sind die Kronen bei denen Zeiten fehlen alle Typ 1.
baum$Kronentyp[is.na(baum$Kronentyp)] <- 1
...
lm.b3 <- glm(Kronenrest ~ Volumen_stk + Kronentyp, data = baum[baum$Verlauf_Kronenrest == "N",])
summary(lm.b3)
